...
from .utils import calcular_raiz, buscar_intervalos_sugeridos
...
def sugerir_intervalos_view(request):
    func = request.GET.get('funcion', '').strip()
    intervalos = buscar_intervalos_sugeridos(func)
    return JsonResponse({'status': 'success', 'intervalos': intervalos})